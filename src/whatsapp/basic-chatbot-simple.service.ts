...
import { ChatbotServiceInterface } from '../chatbot/interfaces/chatbot-service.interface';
import { ChatbotInstance } from '../admin/entities/chatbot-instance.entity';

@Injectable()
export class BasicChatbotSimpleService implements ChatbotServiceInterface {
  private readonly logger = new Logger(BasicChatbotSimpleService.name);
  ...
  async handleMessage(
    ...
    try {
      this.logger.log(`🤖 BasicChatbotSimpleService procesando mensaje para: ${chatbot?.name}`);
      this.logger.log(`📱 Teléfono: ${phoneNumber}`);
      this.logger.log(`💬 Mensaje: ${message.body || message}`);

      const messageText = typeof message === 'string' ? message : (message.body || message.content || '');
      ...
      // Manejo de sesión simple en memoria
      ...
      else if (this.isInfoRequest(messageText)) {
        response = prompts.information || prompts.info || this.getDefaultInfo(chatbot);
      }
      // Respuesta por defecto
      else {
        response = prompts.default || prompts.fallback || this.generateContextualResponse(messageText, chatbot);
      }
      ...
      return {
        status: 'success',
        response: response,
        service: 'BasicChatbotSimpleService',
      ...
      this.logger.error(`❌ Error en BasicChatbotSimpleService: ${error.message}`);
      return {
        status: 'error',
